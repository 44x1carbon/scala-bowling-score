case class BowlingScoreSheet(str: String) {
  ...
  def calcScore(): Int = {
    ...
    frames.foldRight(bonusState) {
      (optionalFrame, state) =>
        optionalFrame match {
          case Some(Normal(one, two)) => State(state.total + one + two, one, two)
          case Some(Spare(one)) => State(state.total + 10 + state.n, one, 10 - one)
          case Some(Strike()) => State(state.total + 10 + state.n + state.nn, 10, state.n)
          case None => state
        }
    }.total
  }
  ...
  val bonus: Option[BonusBall] = if(hoge.length == 1) None else hoge(1) match {
    case s if s.length >= 2 =>
      val one = if(s.charAt(0) == '-') 0 else if(s.charAt(0) == 'X') 10 else s.charAt(0).toString.toInt
      val two = if(s.charAt(1) == '-') 0 else if(s.charAt(1) == 'X') 10 else if(s.charAt(1) == '/') 10 - one else s.charAt(1).toString.toInt
